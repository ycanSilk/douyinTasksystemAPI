...
// 调试日志
error_log('[B端通知列表] 请求开始 - ' . $_SERVER['REQUEST_URI']);
error_log('[B端通知列表] 请求头: ' . json_encode(getallheaders(), JSON_UNESCAPED_UNICODE));
error_log('[B端通知列表] 查询参数: ' . json_encode($_GET, JSON_UNESCAPED_UNICODE));
...
try {
    ...
    error_log('[B端通知列表] 认证成功，用户ID: ' . $currentUser['user_id']);
} catch (Exception $e) {
    error_log('[B端通知列表] 认证失败: ' . $e->getMessage());
    throw $e;
}
...
error_log('[B端通知列表] 处理参数 - page: ' . $page . ', limit: ' . $limit . ', is_read: ' . ($isReadFilter ?? 'null'));
