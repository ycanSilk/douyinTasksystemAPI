...
// 查询用户封禁状态
error_log('查询用户封禁状态，用户ID: ' . $currentUser['user_id']);
$stmt = $db->prepare(" 
...
try {
    ...
    if (!$dailyStats) {
        ...
        $rejectedCount = 0;
        error_log('当日统计记录创建成功，ID: ' . $dailyStatsId);
    } else {
        $dailyStatsId = $dailyStats['id'];
        $rejectedCount = (int)$dailyStats['rejected_count'];
        error_log('当日统计记录已存在，ID: ' . $dailyStatsId . '，驳回次数: ' . $rejectedCount);
    }
    
    // 4. 校验当日驳回次数
    if ($rejectedCount >= 3) {
        error_log('用户当日驳回次数超过限制，禁止接单');
        Response::error('您今日已被驳回3次，暂时无法接单，请明天再试', $errorCodes['TASK_ACCEPT_REJECT_LIMIT']);
    }
    error_log('用户当日驳回次数未超过限制，继续校验');
    
    // 5. 校验当日弃单次数
    $abandonCount = (int)($dailyStats['abandon_count'] ?? 0);
    error_log('用户当日弃单次数: ' . $abandonCount);
    if ($abandonCount > 3) {
        error_log('用户当日弃单次数超过限制，禁止接单');
        Response::error('您今日已弃单超过3次，暂时无法接单，请明天再试', $errorCodes['TASK_ACCEPT_REJECT_LIMIT']);
    }
    error_log('用户当日弃单次数未超过限制，继续校验');
    
    // 6. 校验最后一次接单时间，需间隔3分钟
    error_log('校验最后一次接单时间');
    ...
    if ($remainingCount <= 0) {
        error_log('任务无剩余名额，禁止接单');
        Response::error('该任务已无剩余名额，无法接单', $errorCodes['TASK_ACCEPT_NO_STOCK']);
    }
    error_log('任务有剩余名额，继续处理');
    
    // 9. 解析推荐评论，分配一个给当前用户
    error_log('解析推荐评论，分配给当前用户');
    ...
    // 12. 更新B端任务进行中数量
    error_log('更新B端任务进行中数量，任务ID: ' . $bTaskId);
    $stmt = $db->prepare(" 
    ...
    error_log('当日接单统计更新成功');
    
    // 14. 更新用户任务静态记录的最后接单时间
    error_log('更新用户任务静态记录的最后接单时间');
