...
/**
 * 我租到的订单列表
 * GET /api/rental/orders/my-buyer
 * 
 * 查询参数：
 * - page: 页码，默认1
 * - limit: 每页数量，默认10
...
try {
    ...
    $conditions[] = "(ro.buyer_user_type = ? OR ro.buyer_user_type = ?)";
    $params[] = $userId;
    $params[] = $userTypeStr;
    ...
    foreach ($orders as &$order) {
        ...
        // 是否可续租
        $order['can_renew'] = ($order['status'] == 2 && $order['allow_renew'] == 1);
        ...
        // 剩余天数（如果订单进行中）
        if ($order['status'] == 2 && $order['end_time']) {
            $remainingSeconds = $order['end_time'] - time();
            $order['remaining_days'] = max(0, ceil($remainingSeconds / 86400));
            $order['remaining_hours'] = max(0, ceil($remainingSeconds / 3600));
        } else {
            $order['remaining_days'] = null;
            $order['remaining_hours'] = null;
        }
    }
