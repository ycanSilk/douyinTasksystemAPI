...
if (typeof window.WebSocketManager === 'undefined') {
    window.WebSocketManager = class WebSocketManager {
        constructor() {
            ...
            this.logBufferSize = 10; // 缓冲区大小
            this.logSendInterval = 5000; // 日志发送间隔（毫秒）
            this.logSendTimer = null; // 日志发送定时器
            this.init();
        }
        ...
        sendLogs() {
            ...
            // 发送日志到后端
            fetch('api/admin_notifications/save-log.php', {
                method: 'POST',
                headers: {
        ...
        connect() {
            try {
                ...
                this.log('开始建立WebSocket连接: ' + wsUrl);
                
                // 创建WebSocket连接
                this.socket = new WebSocket(wsUrl);
                
                if (!this.socket) {
                    this.log('WebSocket连接创建失败', 'error');
                    showToast('WebSocket连接失败', 'error');
                    return;
                }
                
                this.log('WebSocket连接对象创建成功: ' + this.socket);
                
                // 设置onopen事件处理程序
                this.socket.onopen = (event) => {
                    this.log('WebSocket连接已打开: ' + JSON.stringify(event));
                    this.connected = true;
                    this.reconnectAttempts = 0;
                ...
                this.socket.onmessage = (event) => {
                    this.log('WebSocket收到消息: ' + event.data);
                ...
                this.log('WebSocket连接异常: ' + error, 'error');
                this.connected = false;
                this.attemptReconnect();
            }
        }
        ...
        attemptReconnect() {
            if (this.reconnectAttempts < this.maxReconnectAttempts) {
                ...
                this.log('尝试重新连接 (' + this.reconnectAttempts + '/' + this.maxReconnectAttempts + ')，延迟 ' + delay + 'ms');
                
                setTimeout(() => {
                    this.connect();
                }, delay);
            } else {
                this.log('达到最大重连次数，停止尝试', 'error');
            }
        }
        ...
        handleAuditNotification(data) {
            if (data.detection_result) {
                ...
                this.log('收到审核通知: ' + JSON.stringify(detectionResult));
                ...
                if (hasNewTasks) {
                    ...
                    this.log('有新审核任务: ' + JSON.stringify(detectionResult));
                    showToast('有新审核任务', 'success');
                } else {
                    this.log('没有新审核任务');
                }
            } else {
                this.log('审核通知数据格式错误', 'error');
            }
        }
        ...
        tryAudioContextPlay() {
            try {
                ...
                console.error('尝试AudioContext失败:', e);
            }
        }
    }
}
