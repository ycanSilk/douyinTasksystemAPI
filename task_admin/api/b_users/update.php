...
/**
 * B端用户更新
 * POST /task_admin/api/b_users/update.php
 */
...
try {
    ...
    if (isset($input['organization_leader'])) {
        $updates[] = 'organization_leader = ?';
        $params[] = trim($input['organization_leader']);
    }
    ...
    $params[] = $id;
    $sql = "UPDATE b_users SET " . implode(', ', $updates) . " WHERE id = ?";
